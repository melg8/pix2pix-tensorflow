...
parser.add_argument("--scale_size", type=int, default=1024, help="scale images to this size before cropping to 512x512")
...
def load_examples():
	...
	scale_size_jitter_float = tf.cast(scale_size_jitter, dtype=tf.float32)

        offset = tf.cast(tf.floor(tf.random_uniform([2], 0, scale_size_jitter_float[0] - CROP_SIZE + 1, seed=seed)), dtype=tf.int32)
...
def main():
    ...
    with tf.name_scope("contrast_summary"):
         tf.summary.image('input_target_output',
                          tf.concat([converted_inputs, converted_outputs, converted_targets], 2),
                          max_outputs=4)
    ...
    tf.summary.scalar("generator_loss_total", model.gen_loss_GAN * a.gan_weight + model.gen_loss_L1 * a.l1_weight)
